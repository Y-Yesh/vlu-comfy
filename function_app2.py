...
def create_color_grade(req: func.HttpRequest) -> func.HttpResponse:
    try:
        ...
        prompt = form.get("prompt", "")
        # Determine whether the sunset option is explicitly provided or implied by the prompt itself.
        sunset_field_value = form.get("sunset")
        if sunset_field_value is None and hasattr(req, "params"):
            sunset_field_value = req.params.get("sunset")
        explicit_sunset = False
        if sunset_field_value is not None:
            explicit_sunset = str(sunset_field_value).lower() in ("true", "1", "yes", "on")
        prompt_sunset = "sunset" in prompt.lower()
        sunset_option = explicit_sunset or prompt_sunset
        
        
        logging.info(f"Prompt: {prompt}")
        logging.info(f"Sunset option: {sunset_option} (explicit={explicit_sunset}, prompt_contains={prompt_sunset})")
        #if not prompt:
            #return func.HttpResponse("Missing 'prompt' field.")
        ...
        # If sunset option is selected, generate 4 variations
        logging.info(f"Checking sunset_option before if statement: {sunset_option} (type: {type(sunset_option)})")
        if sunset_option is True:
            logging.info("Entering sunset option branch - will generate 4 variations")
            variations = []
            try:
                for i in range(4):
                    ...
                    logging.info(f"Open AI Response: {response.output_text}")
                    payload["prompt"] = response.output_text
                    variation_payload = payload.copy()
                    if i == 0:
                        variation_payload["seed"] = 42
                    else:
                        variation_payload["seed"] = random.randint(1, 1000000)
                    
                    logging.info(f"Generating variation {i+1}/4 with seed {variation_payload['seed']}")
                    response = requests.post(url, json=variation_payload, headers=api_headers)
                    response_data = response.json()
                    logging.info(f"Flux Response for variation {i+1}: {response_data}")
                    
                    if "polling_url" in response_data:
                        polling_url = response_data["polling_url"]
                        logging.info(f"Polling URL for variation {i+1}: {polling_url}")
                        ...
                        if result_image:
                            ...
                            variations.append({
                                "variation": i + 1,
                                "seed": variation_payload["seed"],
                                "image_data": base64.b64encode(png_bytes).decode('utf-8'),
                                "filename": out_name
                            })
                            logging.info(f"Variation {i+1} completed successfully")
                        else:
                            logging.error(f"No result image for variation {i+1}")
                            # Fallback to original processed image
                            png_bytes, out_name = _to_png_bytes(byte_converted_image)
                            variations.append({
                                "variation": i + 1,
                        ...
                        logging.error(f"No polling_url for variation {i+1}")
                        # Fallback to original processed image
                        png_bytes, out_name = _to_png_bytes(byte_converted_image)
                        variations.append({
                            "variation": i + 1,
            ...
            # Return JSON response with all 4 variations
            response_data = {
                "variations": variations,
                "total_variations": len(variations),
                "processing_time": total_function_time
            }
            ...
            # Original behavior: single image response
            # Generate prompt using OpenAI
            response = client.responses.create(
                model="gpt-4.1-mini",
                input=[
